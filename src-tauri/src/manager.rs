...
use tauri::{AppHandle, Emitter, Manager as _, Runtime, Wry};
...
pub struct Manager<R: Runtime = Wry> {
    app: AppHandle<R>,
    cfg_path: PathBuf,
    run_path: PathBuf,
    cfg: Mutex<AppConfig>,
...
impl<R: Runtime> Manager<R> {
    ...
    /// 指定配置目录，便于测试
    pub fn new_in(app: AppHandle<R>, dir: PathBuf) -> Arc<Self> {
        ...
        thread::spawn(move || loop {
            ...
            let _ = flusher.app.emit("logs", batch);
        });
    ...
    fn persist_running(&self) {
        ...
        let list: Vec<RunningEntry> = procs
            ...
            .filter_map(|(id, p)| {
                cfg.services.iter().find(|s| s.id == *id).map(|s| RunningEntry {
                    id: id.clone(),
                    pid: p.pid,
                    pgid: p.pgid,
                    cmd: s.cmd.clone(),
    ...
    fn changed(&self) {
        let _ = self.app.emit("services-changed", ());
    }
    ...
    fn adopt(self: &Arc<Self>, svc: ServiceConfig, mut child: Child) {
        ...
        self.log(&svc.id, "INFO", format!("[{}] 已启动 · PID {}", svc.name, pid));
        self.changed();
    ...
    fn on_exit(self: &Arc<Self>, svc: ServiceConfig, generation: u64, code: Option<i32>, manual: bool) {
        let mut should_retry = false;
        {
            let mut procs = self.procs.lock().unwrap();
            let Some(slot) = procs.get_mut(&svc.id) else { return };
        ...
        thread::spawn(move || {
            ...
            m.log(
                &svc.id,
                "WARN",
                format!("[{}] {} 秒后自动重启（第 {n}/{MAX_RESTARTS} 次）", svc.name, wait.as_secs()),
            );
            thread::sleep(wait);
    ...
    fn fail(&self, id: &str, msg: String) {
        ...
        let name = self.find(id).map(|s| s.name).unwrap_or_else(|| id.to_string());
    ...
    pub fn stop(self: &Arc<Self>, id: &str) {
        ...
        thread::spawn(move || {
            if !svc.stop.trim().is_empty() {
                ...
                m.log(&svc.id, "WARN", format!("[{}] 停止命令未生效，改发 SIGTERM", svc.name));
            }
            unsafe { libc::kill(-pgid, libc::SIGTERM) };
            if m.wait_gone(&svc.id, Duration::from_secs(5)) {
                return;
            }
            m.log(&svc.id, "WARN", format!("[{}] 未响应 SIGTERM，改发 SIGKILL", svc.name));
    ...
    pub fn snapshot(&self) -> Snapshot {
        ...
        // 只取 CPU 与内存，跳过磁盘等额外采集；没有服务在跑时连全系统遍历也省掉
        ...
        if too_soon {
            ...
            sys.refresh_processes_specifics(
                ProcessesToUpdate::Some(&[Pid::from_u32(own_pid)]),
                false,
                kind,
            );
        }
        ...
        // 先算出每个在跑的服务的进程树，再用一次 lsof 把监听端口一起查出来
        let trees: HashMap<String, Vec<Pid>> = services
            .iter()
            .filter_map(|svc| {
                let p = procs.get(&svc.id)?;
                if p.state != RunState::Running || p.pid == 0 {
                    return None;
                }
                Some((svc.id.clone(), tree_pids(&kids, Pid::from_u32(p.pid))))
            })
            .collect();
...
fn listening_ports(pids: &[u32]) -> HashMap<u32, Vec<u16>> {
    ...
    let Ok(o) = Command::new("lsof")
        .args(["-nP", "-F", "pn", "-a", "-iTCP", "-sTCP:LISTEN", "-p", &list])
        .output()
    else {
        return out;
    };
...
/// 收集进程树上的全部 pid
fn tree_pids(kids: &HashMap<Pid, Vec<Pid>>, root: Pid) -> Vec<Pid> {
