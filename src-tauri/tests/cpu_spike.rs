...
use hestia_lib::types::{ServiceConfig, Snapshot};
use tauri::test::{mock_builder, mock_context, noop_assets};

fn setup(tag: &str) -> (tauri::App<tauri::test::MockRuntime>, std::sync::Arc<Manager<tauri::test::MockRuntime>>, std::path::PathBuf) {
...
fn cpu(s: &Snapshot, id: &str) -> f32 {
    s.services.iter().find(|x| x.id == id).map(|x| x.cpu).unwrap_or(0.0)
}
...
fn back_to_back_snapshots_reuse_the_last_sample() {
    let (_app, m, dir) = setup("throttle");
    m.save_service(ServiceConfig {
        id: "burn".into(), name: "burn".into(), proj: "t".into(), ic: "chip".into(),
        cmd: "while :; do :; done".into(), stop: String::new(), cwd: String::new(),
        port: 0, auto_restart: false, env: vec![],
    });
    m.start("burn");
...
fn tree_sum_scales_with_busy_process_count() {
    let (_app, m, dir) = setup("cores");

    // 一个忙循环 vs 四个并行忙循环
    for (id, n) in [("one", 1), ("four", 4)] {
        let body = (0..n).map(|_| "(while :; do :; done) &").collect::<Vec<_>>().join(" ");
        m.save_service(ServiceConfig {
            id: id.into(), name: id.into(), proj: "t".into(), ic: "chip".into(),
    ...
    assert!(cpu(&s, "four") > cpu(&s, "one") * 2.0, "四个忙循环应当明显高于一个");
    println!("单个忙循环：{:.0}%", cpu(&s, "one"));
    println!("四个忙循环：{:.0}%", cpu(&s, "four"));
    println!("本机逻辑核心数：{}", std::thread::available_parallelism().map(|n| n.get()).unwrap_or(0));
