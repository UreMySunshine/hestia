...
use hestia_lib::types::{RunState, ServiceConfig};
...
fn setup(tag: &str) -> (App<MockRuntime>, Arc<Manager<MockRuntime>>, PathBuf) {
...
fn starts_captures_output_and_reaps_children() {
    ...
    wait_for("状态转为已停止", Duration::from_secs(20), || !m.is_running("t1"));
...
fn first_run_starts_with_no_services() {
    ...
    let cfg = m.config();
    assert!(cfg.services.is_empty(), "首次运行不应有服务，实际 {}", cfg.services.len());
...
fn config_survives_restart_of_manager() {
    ...
    let m2 = Manager::new_in(app2.handle().clone(), dir.clone());
...
fn reaps_orphans_left_by_a_previous_run() {
    let (_app, m, dir) = setup("orphan");
    // 独特的时长当标记，便于用 pgrep 定位
    m.save_service(svc("t5", "sleep 918274", false));
    m.start("t5");

    wait_for("进程启动", Duration::from_secs(40), || m.is_running("t5"));
    ...
    let app2 = mock_builder().build(mock_context(noop_assets())).unwrap();
    let _m2 = Manager::new_in(app2.handle().clone(), dir.clone());
...
fn does_not_kill_when_command_line_differs() {
    ...
    std::fs::write(
        ...
        format!(
            r#"[{{"id":"ghost","pid":{pid},"pgid":{pid},"cmd":"这条命令根本对不上"}}]"#
        ),
    )
    .unwrap();

    let app = mock_builder().build(mock_context(noop_assets())).unwrap();
    let _m = Manager::new_in(app.handle().clone(), dir.clone());

    std::thread::sleep(Duration::from_millis(800));
    assert!(
        pgrep("sleep 918275") > 0,
        "命令行对不上时不应该动这个进程"
    );
